...
def deux_cond_group_by_column2_count(df, column, col2, col, cond, column2, cond2):
	new_df = df[df[col] == cond]
	n_df = new_df[new_df[column2]== cond2]
	data = n_df.groupby([column, col2])[column].count()
	label1_list = list(data.index.get_level_values(column).unique())
	label2_list = list(data.index.get_level_values(col2).unique())
	sources, targets, vals, p1, p2 = extract(data)
	l1 = encode(label1_list, sources, 0)
...
def sankey_diagram_reg_scat(df, cond, cond2):
	...
	# create labels
	source, target, values, label, p1, p2 = deux_cond_group_by_column2_count(df, 'region', 'sousCategorie', 'groupe', cond, 'categorie', cond2)
